...
export default class SeasonHistory extends Component {
  ...
  onSort(event, sortKey) {
    ...
    this.setState({ data, sorter: this.state.sorter + 1 })
  }
         ...
         pastPops.forEach(function(pop){
           past.forEach(function(playerPast){
              if (pop.season_name === playerPast.season_name){
                ...
                playerPast.season_number = seasonNumber;
        ...
        <div className="input-group-append">
          ...
          {finalData.length > 0 &&
            ...
            <table className="table">
              <thead>
                <tr>
                <th onClick={e => this.onSort(e, 'season_number')} style={{ cursor: 'pointer', 'box-shadow': 'inset 0 -3px 0 0 green' }}>Season Number</th>
                ...
                {finalData.map(function (account, index) {
                  return (
                    <tr key={index} data-item={account}>
                      <td data-title="Season">{account.season_number}</td>
